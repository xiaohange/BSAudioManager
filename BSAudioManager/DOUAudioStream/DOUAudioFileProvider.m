...
static NSString *__douAudioCacheDir = nil;          // 音频缓存路径

static NSUInteger SendProvideDateMinSize = (48 * 1024);
static NSUInteger MinStartParserSize  = (24 * 1024);
...
{
    id<DOUAudioFile> _auidoFile;
    NSUInteger _lastCacheReciveLength;          // 记录上一次下载的数据长度
    NSUInteger _sendProvideDateMinSize;
    NSUInteger _minStartParserSize;
}
...
- (instancetype)_initWithAudioFile:(id <DOUAudioFile>)audioFile
{
    ...
    if (self) {
        ...
        _sendProvideDateMinSize = SendProvideDateMinSize;
        _minStartParserSize = MinStartParserSize;
        
        _is2GNetWork = ([[JQNetworkListener shareInstance] currentNetWorkType] == NetWork2G);
        if (_is2GNetWork) {
            _minStartParserSize = (32 * 1024);
            _sendProvideDateMinSize = (64 * 1024);
        }
...
- (void)_requestDidReceiveResponse
{
    ...
    if ([_audioFile respondsToSelector:@selector(cachePath)]) {
        _cachedPath = [_audioFile cachePath];
        if (!_cachedPath.length) {
...
- (void)_requestDidReceiveData:(NSData *)data
{
    ...
    int minDataSize = _minStartParserSize;
    if (minDataSize > _expectedLength) {
        minDataSize = _expectedLength;
    }
    if (!_readyToProducePackets && !_failed && !_requiresCompleteFile && _receivedLength >= minDataSize) {
    ...
    if ( !_requiresCompleteFile && _receivedLength - _lastCacheReciveLength >= _sendProvideDateMinSize ) {
        _lastCacheReciveLength = _receivedLength;
        if (_is2GNetWork) {
            _sendProvideDateMinSize += (12 * 1024);
            if (_sendProvideDateMinSize > (128 * 1024)) {
                _sendProvideDateMinSize = (128 * 1024);
            }
        }
        [self _invokeEventBlock];
    }
...
+ (NSString *)getDefautlCacheDir
{
    if (!__douAudioCacheDir) {
        // 获取包名
        NSString *appName = [[NSBundle mainBundle] bundleIdentifier];
        appName = [appName stringByAppendingFormat:@".audio"];
        NSString *dirPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"audio"];
        __douAudioCacheDir = dirPath;
    }
    NSFileManager *fileMgr = [NSFileManager defaultManager];
    if (![fileMgr fileExistsAtPath:__douAudioCacheDir]) {
        [fileMgr createDirectoryAtPath:__douAudioCacheDir withIntermediateDirectories:YES attributes:nil error:nil];
    }
    return __douAudioCacheDir;
}

+ (void)setDefaultCacheDir:(NSString *)cacheDir
{
    if (!cacheDir.length) {
        return;
    }
    __douAudioCacheDir = cacheDir;
    NSFileManager *fileMgr = [NSFileManager defaultManager];
    if ([fileMgr fileExistsAtPath:__douAudioCacheDir]) {
        [fileMgr createDirectoryAtPath:__douAudioCacheDir withIntermediateDirectories:YES attributes:nil error:nil];
    }
...
+ (NSString *)_cachedPathForAudioFileURL:(NSURL *)audioFileURL
{
    ...
    return [[[self class] getDefautlCacheDir] stringByAppendingPathComponent:filename];
}
